...
```{r ans13}
meanEcon <- sum(grade * fromEcon) / sum(sumEcon)
meanEcon
```
...
```{r ans22}
submitTime <- ymd_hms(submitTime, tz="Asia/Taipei")
twTime <- submitTime
twTime
```
...
```{r ans23}
nightOwl <- hour(twTime)
nightOwl <- hour(twTime)<=6 & hour(twTime)>=1
nightOwl
```
...
head(isgradepass)
earnedCredit <- as.integer(isgradepass * credit)
earnedCredit
...
```{r ans33}
earnedCredit100ChosenStudent<- sum( studentID == "046S6f645" & schoolYear == "100" & earnedCredit)
...
```{r ans34}
a <- studentID == "046S6f645" & schoolYear == "100" & credit > 0
gpa100ChosenStudent <- sum(a * grade) / credit100ChosenStudent
gpa100ChosenStudent
```
