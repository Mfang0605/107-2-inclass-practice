...
```{r ans12}
names(studentRecord[[4]])[c(1)] <- c("一")
names(studentRecord[[4]])[c(2)] <- c("二")
names(studentRecord[[4]])[c(3)] <- c("三")
names(studentRecord[[4]])[c(4)] <- c("四")
studentRecord
```
...
```{r ans23}
length(commitCheck)
names(commitCheck) <- c(str_c(c("commit"),c(1:30)))
commitCheck
```
...
```{r ans26}
hour_time <- hour(allCommitDatesTW)
minute_time <- minute(allCommitDatesTW)
logicWithinTimeFrame <- (hour_time >= 4 & hour_time <=5 & minute_time >=30 ) | (hour_time >=5 & hour_time <=6 & minute_time <=30 )
...
```{r ans33}
transcriptPartialData <- c(transcriptData[1],transcriptData[7],transcriptData[8])
...
transcriptPartialDataEcon <- list("學號" = c(transcriptPartialData[1]),
                                "學分數" = c(transcriptPartialData[2]),
                        "必選修類別 (必/選/通)" =c(transcriptPartialData[3]))
