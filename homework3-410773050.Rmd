...
```{r ans13}
# only1LossCompanies
logiloss1 <- finStatement$`營業利益（損失）` > 0 & finStatement$`基本每股盈餘（元）` < 0
logiloss2 <- finStatement$`營業利益（損失）` < 0 & finStatement$`基本每股盈餘（元）` > 0
logiloss3 <- logiloss1 | logiloss2
only1LossCompanies <- finStatement$公司名稱[logiloss3] 
...
```{r ans14}
# subsample
subsample <- data.frame(finStatement[logiloss3,"公司名稱"])
subsample
```
...
```{r ans22}
# df_oneShow
df_oneShow <- data.frame(title = c(exhibitionList[[1]][["title"]]),
  ...
  sourceWebName = c(exhibitionList[[1]][["sourceWebName"]]),stringsAsFactors = F
              )
...
#  dataframe_show_info
dataframe_show_info <- function(exhibitionInput){
  df_oneShow <- data.frame(title = c(exhibitionInput[["title"]]),
  ...
  sourceWebName = c(exhibitionInput[["sourceWebName"]]),stringsAsFactors = F
              )
  return(df_oneShow)
}
