...
```{r ans4}
# passData
passgrade <- data3[[1]] >=60
passData <- map(data3,~.[passgrade])
passData
```
...
```{r ans51}
# twDates2
twDate1 <- str_replace(twDates,"民國","")
twDates2 <- str_replace(twDate1,"月","")
twDates2
```
...
```{r ans71}
# failRate
subjectLogic <- sampledTranscriptData[[8]] == "科目204"
grade <-  sampledTranscriptData$學期成績[subjectLogic]
failRate <- sum(grade < 60) / length(grade )
failRate
```
